# AGENTS.md — Agora Data Driven Website
...
scripts/        repo tooling — see scripts/README.md (launchers are MIRRORED from agora-devtools)
```

File map + cookbook for everything under `src/`: [src/README.md](src/README.md).
...
Pushing `main` triggers **TWO independent workflows**: **CI**
([.github/workflows/ci.yml](.github/workflows/ci.yml) — type-check, lint, build checks) and
**Deploy** ([.github/workflows/deploy.yml](.github/workflows/deploy.yml) — the actual Cloud Run
deploy). They do not gate each other — **Deploy can be green while CI is red**, and for a while it
was exactly that (CI failed on every content push over prettier debt; fixed, CI green again since
2026-07-29 — keep it green). Treat every push to `main` as a production deploy and verify both
runs with `gh run list --limit 3`.
